...
echo "Second arg ${1%.*}_av1_2.mkv"
 ...
 echo ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if [ "$(mediainfo "$1" | grep 'Overall bit rate  ' | grep 'Mb/s' -o)"  = "Mb/s" ]; then 
      ...
      echo ffmpeg -hwaccel vaapi -extra_hw_frames 10 -i "$1" -c:v av1_amf -usage 5 -quality 0 -latency 3 -rc 3 -b:v "$(($test * $compression))"M "${1%.*}_av1_2.mkv"
      echo ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      $(ffmpeg -hwaccel vaapi -extra_hw_frames 10 -i "$1" -c:v av1_amf -usage 5 -quality 0 -latency 3 -rc 3 -b:v "$(($test * $compression))"M "${1%.*}_av1_2.mkv")
      ...
      echo ffmpeg -hwaccel vaapi -extra_hw_frames 10 -i "$1" -c:v av1_amf -usage 5 -quality 0 -latency 3 -rc 3 -b:v "$(($test * $compression))"k "${1%.*}_av1_2.mkv"
      echo ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      $(ffmpeg -hwaccel vaapi -extra_hw_frames 10 -i "$1" -c:v av1_amf -usage 5 -quality 0 -latency 3 -rc 3 -b:v "$(($test * $compression))"k "${1%.*}_av1_2.mkv")
